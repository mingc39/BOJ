...
// 곱 구하기(현재 확인할 index 번호, 현재 index의 범위 시작, 끝, 합을 구하려는 범위의 시작, 끝) 
long long sum(int index, int istart, int iend, int rstart, int rend) { 
	if(istart > rend || iend < rstart) return 1; // index의 범위와 구하려는 범위가 전혀 겹치지 않는 경우  
				 ...
				 sum(index * 2 + 1, (istart + iend) / 2 + 1, iend, rstart, rend)) % 1000000007;  // 자식 노드 탐색  
}
